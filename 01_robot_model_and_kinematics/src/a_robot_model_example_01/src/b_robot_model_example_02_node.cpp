...
public:
  cRobotModel()
      : nh_(), robot_model_loader_("robot_description"),
    ...
    joint_state_subs_ = nh_.subscribe<sensor_msgs::JointState>(
        "joint_states", 5, &cRobotModel::setJointStates, this);
  }
  void printDK() {
    /*This function does the follong
     * 1. Get the joint values of the robot
     * 2. print the joint names
     * 3. print the link names and their poses*/
    joint_mutex_.lock();
    const std::vector<std::string> &variable_names =
        joint_group_->getVariableNames();
    const std::vector<std::string> &link_names =
        joint_group_->getLinkModelNames();
    ...
    // print the link names and their poses
    for (const std::string &lname : link_names) {
      // NOTE: The function getGlobalLinkTransform calls updateLinkTransforms
      // which is the function which computes the forward kinematics.
      const Eigen::Isometry3d &link_pose =
          kinematic_model_->getGlobalLinkTransform(lname);
